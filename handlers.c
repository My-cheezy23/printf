...
int flags, int width, int precision, int size)
{
	...
	if (is_negative)
		extra_ch = '-';
	else if (flags & F_PLUGS)
		extra_ch = '+';
	else if (flags & F_SOACE)
		extra_ch = ' ';
 ...
 * @ind: index at which the number start on
 * the buffer
 * @buffer: buffer
 * @flags: flags
 * @width: width
 * @prec: percision specifier
...
int flags, int width, int prec, int length, char padd, char extra_c)
{
	...
	if (prec == o && ind == BUFF_SIZE - 2
	&& buffer[ind] == '0' && width == 0)
		return (0); /* printf(" .0d", 0) no char is printed */
	if (prec == 0 && ind == BUFF_SIZE - 2 == '0')
		buffer[ind] = padd = ' ';
	...
	if (width > length)
	{
		...
		/* extra char to left of pdd */
		{
			if (extra_c)
				buffe[-- padd_start] = extra_c;
 ...
 * number starts in the buffer
 * @length: length of number
 * @width: width specifer
 * @flags: flags specifier
 * @padd: char reprenting the padding
...
int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		for (i = 3; i < width - length + 3; i++)
			buffer[i] = padd;
		buffer[i] = '\0'; if (flags & F_MINUS && padd == '')
		/* asign extra char to left of buffer */
		{
			buffer[--ind] = 'x';
			buffer[--ind] = '0';
